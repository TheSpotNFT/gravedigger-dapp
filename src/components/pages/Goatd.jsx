...
export const Goatd = ({
  ...
  let ownedFilter = traits.filter((item) => {
    if (
      walletTraits.includes(item.id.toString()) ||
      solidBG.includes(item.id)
    ) {
      return item;
    }
  });
  ...
  //         console.log(traitFetch);
  //         traitFetch().then((data) =>
  //           setTraitsAvailability(JSON.stringify(data))
  //         );
  //       }
  //     },
  //     [chosenTrait]
  ...
  return (
      ...
      <div className="lg:sticky top-20 grid 2xl:grid-cols-3 xl:grid-cols-3 lg:grid-cols-2 md:grid-cols-1 sm:grid-cols-1 gap-4 mt-1 ml-6 sm:p-5 bg-slate-900 lg:pb-3">
        ...
        <div className="grow border-dashed border-4 border-slate-500 p-3 m-1 text-left col-span-1 w-96 md:mt-10 lg:mt-1 mt-10 sm:mt-10 text-sm">
          ...
          <div className="font-mono text-white list-none flex pb-3">
            <div
              className={`text-${
                walletTraits.includes(`${chosenTrait.BackgroundID}`) ||
                solidBG.some((ai) => chosenTrait.BackgroundID === ai)
                  ? "spot-yellow"
                  : "[red]"
              } font-bold pr-3`}
            >
          ...
          <div className="font-mono text-white list-none flex pb-3">
            <div
              className={`text-${
                walletTraits.includes(`${chosenTrait.BodyID}`)
                  ? "spot-yellow"
                  : "[red]"
              } font-bold pr-3`}
            >
          ...
          <div className="font-mono text-white list-none flex pb-3">
            <div
              className={`text-${
                walletTraits.includes(`${chosenTrait.HeadID}`)
                  ? "spot-yellow"
                  : "[red]"
              } font-bold pr-3`}
            >
          ...
          <div className="font-mono text-white list-none flex pb-3">
            <div
              className={`text-${
                walletTraits.includes(`${chosenTrait.EyesID}`)
                  ? "spot-yellow"
                  : "[red]"
              } font-bold pr-3`}
            >
          ...
          <div className="font-mono text-white list-none flex pb-3">
            <div
              className={`text-${
                walletTraits.includes(`${chosenTrait.MouthID}`)
                  ? "spot-yellow"
                  : "[red]"
              } font-bold pr-3`}
            >
          ...
          <div className="font-mono text-white list-none flex pb-3">
            <div
              className={`text-${
                walletTraits.includes(`${chosenTrait.HeadwearID}`) ||
          ...
          <div className="pt-1 pb-1 flex">
            ...
            />
            <button
              className="m-2 rounded-lg px-4 py-2 border-2 border-gray-200 text-gray-200
    hover:bg-gray-200 hover:text-gray-900 duration-300 font-mono font-bold text-base"
          ...
          <div className="font-mono text-white list-none flex pb-0 pt-3 text-sm">
            ...
            Traits in your wallet:{" "}
            {(apiLoaded, checkMyTraits && walletTraits.length + " nos.")}{" "}
            {
              (apiLoaded,
              checkMyTraits &&
                "IDs: " + walletTraits.map((trait) => " " + trait))
            }
          </div>
          ...
          <div className="font-mono text-white list-none flex pb-3 text-sm">
            <span
              className={
                traitsAvailability === "0" ? "text-green-300" : "text-[#fa2121]"
              }
            >
              {traitsAvailability === "0" && currentDNA.length >= 14
                ? "Trait Combo is Unique!"
                : null}
              {traitsAvailability === "1" && currentDNA.length >= 14
                ? "Trait Combo's Been Minted!"
                : null}
            </span>
          </div>{" "}
          {/* End of btm text lines */}
        </div>
        {/* Stats div Ends*/}
        {/* SearchBox */}
        <div className="grid grid-rows-1 grid-cols-1 gap-4 pt-10 pl-10 self-end">
          <div className="col-span-1">
            <input
              type="text"
              className="border-2 border-slate-600 bg-slate-400 text-left font-mono placeholder-slate-600 pl-2"
              placeholder="search trait/ID..."
              value={filter}
          ...
          <div className="self-end">
            <button
              className="w-1/3 m-2 rounded-lg px-4 py-2 border-2 border-gray-200 text-gray-200
    hover:bg-gray-200 hover:text-gray-900 duration-300 font-mono font-bold text-base"
              onClick={() => {
                setOwnedCards(!ownedCards);
              }}
            >
              {!ownedCards ? "My Traits" : "All Traits"}
            </button>
          </div>
        </div>
        {/* SearchBox Ends */}
